...
class args_define():
    ...
    parser.add_argument('--use_kl', default=False,type=bool,
                        help="If true, add a KL divergence loss term to the total loss.")
    ...
    parser.add_argument('--encoder', default=0, type=int,
                        help="encoder type")
    parser.add_argument('--seed', default=0, type=int)
    parser.add_argument('--odetype', default="gcn", type=str)
...
if __name__ == '__main__':
    ...
    torch.manual_seed(args.seed) # governs initial weight
    np.random.seed(args.seed) # governs the standard deviation of initial weight 
    ...
    if args.resume_path is None:  # build a new dir if training from scratch
        embedding_save_path = args.data_path + date + f'/seed_{args.seed}/embeddings_ode{args.odetype}_bs{args.batch_size}_ws{args.window_size}_dim{args.out_dim}_kl{args.use_kl}'
    ...
    if not os.path.exists(header):
        os.makedirs(header)
    logger = get_logger(header + f"/encoder_{args.encoder}.log", filepath=os.path.abspath(__file__))
    #########################################
    ...
    if ((args.resume_path is not None) and (args.resume_point == 0) and ( args.resume_current)) or args.resume_path is None:
        if not args.use_dgi:
            train_indices, indices_to_remove, model = KPGNN.initial_maintain(train_i, 0, data_split, metrics,\
                                                                       embedding_save_path, loss_fn, logger=logger)
        else:
            train_indices, indices_to_remove, model = KPGNN.initial_maintain(train_i, 0, data_split, metrics,\
                                                                       embedding_save_path, loss_fn, None, loss_fn_dgi, logger=logger)
        ...
        if ((args.resume_path is not None) and (args.resume_point == i - 1) and (not args.resume_current)) or args.resume_path is None:
            if not args.use_dgi:
                model = KPGNN.infer(train_i, i, data_split, metrics, embedding_save_path, loss_fn, train_indices, model, None,
                              indices_to_remove, logger=logger, args=args)
            else:
                model = KPGNN.infer(train_i, i, data_split, metrics, embedding_save_path, loss_fn, train_indices, model,
                              loss_fn_dgi, indices_to_remove, logger=logger, args=args)
        # Maintain
            ...
            if i % args.window_size == 0:
                ...
                if not args.use_dgi:
                    train_indices, indices_to_remove, model = KPGNN.initial_maintain(train_i, i, data_split, metrics,
                                                                               embedding_save_path, loss_fn, model, logger=logger)
                else:
                    train_indices, indices_to_remove, model = KPGNN.initial_maintain(train_i, i, data_split, metrics,
                                                                               embedding_save_path, loss_fn, model,
                                                                               loss_fn_dgi, logger=logger)
