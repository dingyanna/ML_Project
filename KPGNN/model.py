...
class KPGNN():
    ...
    def infer(train_i, i, data_split, metrics, embedding_save_path, loss_fn, train_indices=None, model=None,
            loss_fn_dgi=None, indices_to_remove=[], logger=None, args=None):
        ...
        if (args.resume_path is not None) and (not args.resume_current) and (
            ...
            if args.use_dgi:
                ...
                model = GAT(g, in_feats, int(args.out_dim/args.num_heads), args.out_dim, args.num_heads, args.use_residual, forward_func=args.encoder, odetype=args.odetype)
            ...
            logger.info("Resumed model from the previous block.")
        ...
        test_nmi = evaluate(extract_features, extract_labels, test_indices, -1, num_isolated_nodes, save_path_i, None, False, logger=logger)
        ...
        for epoch in range(args.n_infer_epochs):
            ...
            test_nmi = evaluate(extract_features, extract_labels, test_indices, epoch, num_isolated_nodes, save_path_i,
                                cluster_centers, False, logger=logger)
            all_test_nmi.append(test_nmi)
            # end one epoch
    ...
    def initial_maintain(train_i, i, data_split, metrics, embedding_save_path, loss_fn, model=None, loss_fn_dgi=None, logger=None):
        ...
        if (args.resume_path is not None) and args.resume_current and (
            ...
            if args.use_dgi:
                ...
                model = GAT(g, in_feats, int(args.out_dim/args.num_heads), args.out_dim, args.num_heads, args.use_residual, forward_func=args.encoder)
            ...
            model_path = embedding_save_path + '/block_' + str(args.resume_point) + '/models/best.pt'
            model.load_state_dict(torch.load(model_path))
            logger.info("Resumed model from the current block.")
        ...
        # get initial kmeans cluster centers
        extract_nids, extract_features, extract_labels = extract_embeddings(g, model, len(labels), labels)
        cluster_centers = run_initial_kmeans(extract_features, extract_labels, train_indices)
        print('initial_cluster_centers:', cluster_centers.shape)
        ...
        for epoch in range(args.n_epochs):
            ...
            for batch_id, nf in enumerate(dgl.contrib.sampling.NeighborSampler(g,
                ...
                if args.use_dgi:
                    ...
                    if args.use_kl:
                        ...
                        loss_kld, cluster_centers = kl_divergence_loss(pred, cluster_centers)
                        # print('loss_kl.item():', loss_kl.item())
                ...
                if batch_id % args.log_interval == 0:
                    ...
                    if args.use_dgi:
                        ...
                        message += '\tLoss_dgi: {:.6f}'.format(np.mean(losses_dgi))
                    if args.use_kl:
                        message += '\tLoss_kld: {:.6f}'.format(np.mean(losses_kld))
                    for metric in metrics:
                        message += '\t{}: {:.4f}'.format(metric.name(), metric.value())
                    logger.info(message)
                    # with open(save_path_i + '/log.txt', 'a') as f:
            ...
            validation_nmi = evaluate(extract_features, extract_labels, validation_indices, epoch, num_isolated_nodes,
                                    save_path_i, cluster_centers, True, logger=logger)
            ...
            if validation_nmi > best_vali_nmi:
                ...
                p = model_path + '/best.pt'
                torch.save(model.state_dict(), p)
                logger.info(f'Best model saved after epoch {str(epoch)}')
            else:
                wait += 1
            if wait == args.patience:
                logger.info('Saved all_mins_spent')
                logger.info(f'Early stopping at epoch {str(epoch)}')
                logger.info(f'Best model was at epoch ')
                break
            # end one epoch

        # Save all validation nmi
        np.save(save_path_i + '/all_vali_nmi.npy', np.asarray(all_vali_nmi))
        np.save(save_path_i + '/train_loss.npy', np.asarray(all_vali_nmi))
        # Save time spent on epochs
        np.save(save_path_i + '/mins_train_epochs.npy', np.asarray(mins_train_epochs))
        logger.info('Saved mins_train_epochs.')
        # Save time spent on batches
        np.save(save_path_i + '/seconds_train_batches.npy', np.asarray(seconds_train_batches))
        logger.info('Saved seconds_train_batches.')
