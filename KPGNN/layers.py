...
class GAT(nn.Module):
    def __init__(self, g, in_dim, hidden_dim, out_dim, num_heads, use_residual=False, forward_func=0, activation=nn.ELU(), odetype="gcn"):
        super(GAT, self).__init__()
        print("in_dim, hidden_dim, out_dim, num_heads" , in_dim, hidden_dim, out_dim, num_heads)
        # in_dim, hidden_dim, out_dim, num_heads 302 8 32 4
        if forward_func == 0:
            ...
            assert(hidden_dim * num_heads == out_dim) 
            self.encoder = MultiHeadGATLayer(in_dim, hidden_dim, num_heads, 'cat', use_residual)
            self.layer1 = MultiHeadGATODE(g, out_dim, out_dim, 1, activation, dropout=0.9, bias=True, merge='cat',   odetype=odetype)
            self.layer2 = MultiHeadGATODE(g, out_dim, out_dim, 1, activation, dropout=0.9, bias=True, merge='cat',  odetype=odetype)
        
        if forward_func == 0:
            self.forward = self.forward_kpgnn 
        elif forward_func == 1: # continuous
            self.odetype = odetype 
            self.forward = self.forward_continuous
        elif forward_func == 2:
            self.forward = self.forward_ae # automorphic equivalence aware 
        else:
            self.forward = self.forward_ae_cont # continuous + automorphic equivalence aware 

    def forward_kpgnn(self, nf, corrupt=False):
        ...
        return h
    # subgraph
    def forward_continuous(self, nf, corrupt=False):
        features = nf.layers[0].data['features']
        if corrupt:
            nf.layers[0].data['h'] = features[torch.randperm(features.size()[0])]
        else:
        ...
        h = self.encoder(nf, 0)
        h = F.elu(h)
        for attd_head in self.layer1.heads:
            #attd_head.norm = nf.layers[1].data['norm']
            attd_head.nf = nf 
            attd_head.layer_id = 1
        
        integration_time = torch.linspace(0,1,10)
        out = torchdiffeq.odeint(self.layer1, h, integration_time, rtol=1e-8, atol=1e-8, method="rk4")
        h = out[-1]
        nf.layers[1].data['h'] = h
        ...
        #     return h
        return h
        # print("finish layer 1")
        # for attd_head in self.layer2.heads:
        #     attd_head.norm = nf.layers[1].data['norm']
        #     attd_head.nf = nf 
        #     attd_head.layer_id = 1
        # out = torchdiffeq.odeint(self.layer2, h, integration_time, rtol=1e-8, atol=1e-8, method="rk4")
        # return h
    
    def forward_ae(self, nf, corrupt=False):
        pass 
    
    def forward_ae_cont(self, nf, corrupt=False):
        pass 
